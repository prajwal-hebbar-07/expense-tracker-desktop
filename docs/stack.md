...
status: active
updated: 2026-08-05
links: [repo-layout, persistence-sqlite, ollama-flow, turborepo, ollama-key-in-settings]
---
...
2. **Business logic and all disk/network access go in Rust** (`apps/desktop/src-tauri/src/lib.rs`), exposed to the frontend with `#[tauri::command]` and called via `invoke("command_name", { arg })`. The WebView is a rendering layer. This keeps file paths and logic out of a context that can execute arbitrary loaded script. One deliberate exception, taken with its reasoning written down: the Ollama API key is read in TypeScript and passed into the command — [[ollama-key-in-settings]].
