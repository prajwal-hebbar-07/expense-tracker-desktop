...
| File | Purpose |
|---|---|
| `macos/Khata.app` | the app itself |
| `macos/Khata.app.tar.gz` | **the update payload** |
| `macos/Khata.app.tar.gz.sig` | its signature, pasted into the manifest |
| `dmg/Khata_<version>_aarch64.dmg` | first-time install |

Then publish a GitHub release whose assets are `Khata.app.tar.gz` and a `latest.json`:
...
```json
{
  ...
  "platforms": {
    "darwin-aarch64": { "signature": "<contents of the .sig file>", "url": "https://github.com/.../releases/download/v0.1.1/Khata.app.tar.gz" }
  }
}
```
...
pnpm tauri build --bundles app,updater && cp -R …/bundle/macos/Khata.app /Applications/
# bump to 0.1.1, rebuild, copy Khata.app.tar.gz + a latest.json into a folder
(cd that-folder && python3 -m http.server 8787)
open -a /Applications/Khata.app     # the banner should appear
```
...
It does not remove Gatekeeper entirely. A **first install** from a downloaded `.dmg` still carries the quarantine attribute, so it needs right-click → Open once, or `xattr -dr com.apple.quarantine /Applications/Khata.app`. The **update** path downloads over HTTP inside the app rather than through a browser, so nothing sets the quarantine attribute — ⚠ verified in principle, not yet observed end to end on a real release. Watch the first one.
