...
3. **Only ever suggest cutting the controllable half.** Essentials are `ESSENTIAL` in `report.ts` (Rent, Loans & EMIs, Groceries, Bills & Utilities, Health) — names from the closed list in [[expense-categories]], typed as `Category` so a rename there breaks the build rather than silently emptying the set. A target below essentials would require missing a basic need or debt payment, which is not a habit.
